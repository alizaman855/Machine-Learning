...
class MinesweeperAI():
    ...
    def make_safe_move(self):
        ...
        """
        print(f"The moved is {self.moves_made}")
        safeList = list([(i,j) for i in range(self.height) for j in range(self.width) if (i,j) not in self.moves_made and (i,j) in self.safes])
        
        if len(safeList) == 0:
            return None

        num = random.randint(0, len(safeList)-1)
        return safeList[num]
    ...
    def make_random_move(self):
        ...
        """
        print(f"The moved before make_random_move is {self.moves_made}")
        safeList = list([(i,j) for i in range(self.height) for j in range(self.width) if (i,j) not in self.moves_made and (i,j) not in self.mines])
        if len(safeList) == 0:
            return None
        if len(safeList) == 0:
            return None
